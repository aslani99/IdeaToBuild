...
- Session persistence اضافه شد: `AuthContext` روی mount اپ، `restoreSession()` رو صدا می‌زنه؛ کاربری که قبلاً وارد شده، دیگه لازم نیست دوباره وارد بشه.
- برای Idea (اولین موجودیت)، یک لایه‌ی ذخیره‌سازی محلی رمزنگاری‌شده کامل ساخته شد: دیتابیس SQLite ساده در Rust + رمزنگاری AES-256-GCM سطح‌فیلد + کلید در Windows Credential Manager (نه SQLCipher، به دلیل پیچیدگی build با OpenSSL روی ویندوز — جزئیات در AD-010).
- `LocalFirstIdeaRepository` جایگزین `SupabaseIdeaRepository` در `useIdeas` شد — همه‌ی خواندن/نوشتن Idea اول محلیه.
- `SyncEngine` ساخته شد: صف تغییرات محلی رو، وقتی آنلاینه (رویداد `online` + یک تایمر دوره‌ای)، به Supabase push می‌کنه.
- **هشدار صادقانه:** کد Rust جدید (`local_store.rs` و وابستگی‌های `Cargo.toml`) در این نشست کامپایل نشده — چون من به کامپایلر Rust دسترسی ندارم. اولین باری که کاربر `pnpm tauri dev` رو بعد از این تغییر اجرا کنه، ممکنه خطای build بگیره (وابستگی‌های جدید مثل `aes-gcm`، `keyring`، `rusqlite` باید دانلود/کامپایل بشن، که ممکنه چند دقیقه طول بکشه یا خطای غیرمنتظره بده) — باید با هم رفعش کنیم، طبق قانون «هرگز ادعای موفقیت بدون تأیید نکن».
